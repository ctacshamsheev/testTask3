...
GasEngine::GasEngine(const Config& cfg, double t_out) : _cfg(cfg) {
  // при старте начинает с нулевой скорости вращения.
  _currentV = 0;
  //Температура двигателя до момента старта должна равняться температуре
  //окружающей среды.
  _currentT = t_out;
  _cfg.setTOut(t_out);
}
...
void GasEngine::nextStep() {
  ...
  if (_currentT >= _cfg._t_over) {
    throw std::range_error(
        "the current temperature exceeds the critical: Tcur = " +
        std::to_string(_currentT) + " Tcr = " + std::to_string(_cfg._t_over));
  }
}

double GasEngine::getT() const { return _currentT; }

double GasEngine::getV() const { return _currentV; }

std::string GasEngine::toString() const {
  std::stringstream ss;
  ss << "[GasEngine]\tT = " << std::to_string(this->getT())
